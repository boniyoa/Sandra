...
document
  .getElementById("buscador-equipos")
...
function actualizarCarrito() {
  ...
  document.getElementById("carrito-total2").textContent = `€${total.toFixed(
    2
  )}`;
  ...
  carrito.forEach((item, idx) => {
    ...
    li.innerHTML = `
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                <span><strong>${item.unidades} unidad${
      item.unidades > 1 ? "es" : ""
    }</strong> de ${item.nombre} <strong>(${item.dias} día${
      item.dias > 1 ? "s" : ""
    })</strong> -<em> €${(item.precio * item.unidades * item.dias).toFixed(
      2
    )}</em></span>
  ...
  lista.querySelectorAll("button[data-editar]").forEach((btn) => {
    btn.onclick = function () {
      ...
      ).find((c) => c.querySelector("h3").textContent === item.nombre);
      if (card) {
        const alquilarBtn = card.querySelector("[data-alquilar]");
        if (alquilarBtn) {
          maxUnidades =
            parseInt(alquilarBtn.getAttribute("data-max-unidades")) || 2;
        }
      }
      // Opciones de unidades
      let opcionesUnidades = "";
      for (let i = 1; i <= maxUnidades; i++) {
        opcionesUnidades += `<option value="${i}" ${
          i === item.unidades ? "selected" : ""
        }>${i}</option>`;
      }
      // Opciones de días (1-30)
      let opcionesDias = "";
      for (let d = 1; d <= 30; d++) {
        opcionesDias += `<option value="${d}" ${
          d === item.dias ? "selected" : ""
        }>${d}</option>`;
      }
      formDiv.innerHTML = `
      ...
      formDiv.querySelector("form").onsubmit = function (ev) {
        ev.preventDefault();
        const nuevasUnidades = parseInt(
          formDiv.querySelector('select[name="unidades"]').value
        );
        const nuevosDias = parseInt(
          formDiv.querySelector('select[name="dias"]').value
        );
        ...
        const idxExistente = carrito.findIndex(
          (it, i) =>
            it.nombre === nombre && it.unidades === nuevasUnidades && i !== idx
        );
  ...
  btn.addEventListener("click", function (e) {
    ...
    modal.querySelector("#añadir-alquilar").onclick = function () {
      ...
      const precio = parseFloat(
        precioTexto.replace("€", "").replace("/día", "").replace(",", ".")
      );

      // Buscar si ya existe el mismo producto con las mismas unidades
      const idxExistente = carrito.findIndex(
        (item) => item.nombre === nombre && item.unidades === unidades
      );
      if (idxExistente !== -1) {
        // Restar el total anterior
        total -=
          carrito[idxExistente].precio *
          carrito[idxExistente].unidades *
          carrito[idxExistente].dias;
...
document
  .getElementById("close-contacto-modal")
...
// Enviar presupuesto
document
  .getElementById("enviar-presupuesto")
  .addEventListener("click", function () {
    ...
    carrito.forEach((item) => {
      const li = document.createElement("li");
      li.textContent = `${item.unidades} unidad${
        item.unidades > 1 ? "es" : ""
      } de ${item.nombre} (${item.dias} día${item.dias > 1 ? "s" : ""}) - €${(
        item.precio *
        item.unidades *
        item.dias
      ).toFixed(2)}`;
...
document
  .getElementById("cerrar-modal-enviar")
  .addEventListener("click", function () {
    document.getElementById("modal-enviar-presupuesto").classList.add("hidden");
  });

document
  .getElementById("modal-enviar-presupuesto")
  .addEventListener("click", function (e) {
    if (e.target === this) {
      this.classList.add("hidden");
    }
...
function sendMail() {
  ...
  if (!nombre || !email || !telefono || !resumen) {
    alert(
      "Por favor, complete todos los campos y asegúrese de que el carrito no esté vacío."
    );
    return;
  }
  ...
  emailjs
    ...
    .then(() => {
      alert(
        "Presupuesto enviado correctamente, en breves nos pondremos en contacto con usted."
      );
